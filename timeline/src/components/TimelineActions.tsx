...
export default function TimelineActions({ locale, items, isMouseDown, isPaneOpen, refresh, onSave, paneChange }: ITimelineActionsProps) {
        ...
        <>
            <div className={`absolute flex top-2 left-2 z-20 ${isMouseDown ? "pointer-events-none" : "pointer-events-auto"}`}>
                <div className='bg-white rounded-[4px] shadow-dynamics flex justify-center items-center mr-1'>
                    <button onClick={() => {resetFilters(); refresh()}} className="relative flex justify-center items-center w-5 h-5 p-1 m-0.5 rounded-[4px] hover:bg-slate-100 duration-200 transition-colors">
