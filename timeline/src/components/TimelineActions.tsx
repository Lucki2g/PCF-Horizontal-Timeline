...
import { ArrowBigLeftDash, ArrowBigRightDash, Filter, PanelRightClose, PanelRightOpen, RefreshCcw } from 'lucide-react';
        ...
        <>
            <div className={`absolute flex top-2 left-2 z-20`}>
                <div className='bg-white rounded-[4px] shadow-dynamics flex justify-center items-center mr-1'>
                    {/* Refresh timeline */}
                    <ActionButton tooltip='Refresh data' onClick={() => {resetFilters(); setState(true)}}>
                    ...
                    <ActionButton tooltip='Go to next item' onClick={animateNext}>
                        <ArrowBigRightDash size={48} strokeWidth={1.5} absoluteStrokeWidth />
                    </ActionButton>
                    <div className='w-px h-full bg-gray-300 mx-0.5' />
                    {/* Filter */}
                    <ActionButton tooltip='Filter items' onClick={() => showDialog(<FilterDialog items={items} onSave={onSave} />, "w-[540px]")}>
                    ...
                    <ActionButton tooltip='Show timeless items' onClick={paneChange}>
                        {
                            ...
                            <PanelRightClose size={48} strokeWidth={1.5} absoluteStrokeWidth />
                        }
                    </ActionButton>
                </div>
            </div>
